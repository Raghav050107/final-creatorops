...
import { Mail, Search, LogIn, ShieldCheck, LogOut, KeyRound } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-16 bg-surface border-b border-border px-6 flex items-center justify-between sticky top-0 z-20 shadow-subtle">
      {/* Title & Cloud Status Badge */}
      <div className="flex items-center gap-3">
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] font-bold">
          ...
          <span>{agency ? agency.name : 'Unseen Hours'} Cloud</span>
        </div>

        {overdueCount > 0 && (
          <span className="inline-flex items-center gap-1 text-[11px] font-semibold bg-warn-bg text-warn px-2.5 py-0.5 rounded-full border border-warn-border">
            <span className="w-1.5 h-1.5 rounded-full bg-warn animate-pulse" />
            {overdueCount} {overdueCount === 1 ? 'deliverable' : 'deliverables'} overdue
          </span>
        )}
      </div>
      ...
      <div className="relative w-64 md:w-72">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-1.5 bg-bg text-ink text-xs rounded-md border border-border focus:outline-none focus:border-accent focus:bg-white transition-all placeholder:text-slate-400"
        />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {openAuthModal && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold text-accent bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-3 py-1.5 rounded-md transition-colors shadow-subtle"
